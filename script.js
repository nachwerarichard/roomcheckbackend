...
async function login() {
  ...
  try {
    ...
    if (!res.ok) {
      showMessage('loginMsg', 'Invalid username or password.', true);
      return;
    }
    ...
    if (hasRole('admin') || hasRole('housekeeper')) {
      showTab('checklist');
    } else if (hasRole('storemanager')) {
      showTab('inventory'); // ✅ Correct default for Store Manager
    }
  } catch (err) {
    console.error('Login error:', err);
    showMessage('loginMsg', 'Server error during login.', true);
  }
}
...
function logout() {
  ...
  hide(roomChecklistSection);
  hide(housekeepingReportSection);
  hide(inventorySection);
}

// Role-based front-end access control
function updateUIVisibility(role) {
  // Hide all targeted elements first
  adminOnlyElements.forEach(hide);
  storeManagerOnlyElements.forEach(hide);
  housekeeperOnlyElements.forEach(hide);

  // Show tab buttons or hide as needed
  // Default: hide all
  ...
  switch (role) {
    case 'admin':
      adminOnlyElements.forEach(show);
      storeManagerOnlyElements.forEach(show);
      housekeeperOnlyElements.forEach(show);
    ...
    case 'housekeeper':
      ...
      // ✅ Tabs: Checklist + Housekeeping; hide Inventory
      tabChecklistBtn?.classList.remove('hidden');
      tabHousekeepingBtn?.classList.remove('hidden');
      break;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentUserRole) {
    ...
    if (hasRole('admin') || hasRole('housekeeper')) {
      ...
      showTab('inventory'); // ✅ fixed here as well
    }
  } else {
    document.getElementById('mainApp').style.display = 'none';
  }

  // If Missing Items date filter exists, wire it
...
async function saveStatusReport(id) {
  if (currentUserRole !== 'admin') {
    showMessage('statusMessage', 'You do not have permission to save reports.', true);
    return;
  }
  const updatedData = {
...
function renderInventoryTable(inventoryData, isSnapshot = false) {
  ...
  filteredInventory.forEach((it) => {
    ...
    if (Number(it.quantity) <= Number(it.lowStockLevel)) {
      tr.classList.add('low-stock-warning'); // Style in CSS
    }
...
function editInventoryItem(id) {
  if (currentUserRole !== 'admin') {
    showMessage('inventoryMessage', 'You do not have permission to edit inventory.', true);
    return;
  }
...
async function saveInventoryItem(id) {
  if (currentUserRole !== 'admin') {
    showMessage('inventoryMessage', 'You do not have permission to save inventory.', true);
    return;
  }
...
async function deleteInventoryItem(id) {
  if (currentUserRole !== 'admin') {
    showMessage('inventoryMessage', 'You do not have permission to delete inventory.', true);
    return;
  }
...
// Expose functions used by inline onclick (if bundling, attach to window)
window.login = login;
